...
def get_data(filters: Filters, attendance_map: dict) -> list[dict]:
	...
	if filters.group_by:
		...
		for value in group_by_param_values:
			...
			records = get_rows(
				employee_details[value], filters, employee_holiday_map, shift_holiday_map, attendance_map
			)
		...
		data = get_rows(employee_details, filters, employee_holiday_map, shift_holiday_map, attendance_map)
...
def get_shift_holiday_map(attendance_map: dict, filters: Filters) -> dict[str, list[dict]]:
	...
	shift_rows = (
		...
		.select(ShiftType.name, ShiftType.holiday_list)
		.where(ShiftType.name.isin(list(shift_names)))
		.where(ShiftType.holiday_list.isnotnull())
		.where(ShiftType.holiday_list != "")
	).run(as_dict=True)

	shift_holiday_lists = {row.name: row.holiday_list for row in shift_rows}
	...
	holiday_rows = (
		frappe.qb.from_(Holiday)
		.select(Holiday.parent, Holiday.holiday_date, Holiday.weekly_off)
		.where(Holiday.parent.isin(list(set(shift_holiday_lists.values()))))
		.where(Holiday.holiday_date.between(start_date, end_date))
	).run(as_dict=True)

	hl_holidays = {}
	for holiday in holiday_rows:
		hl_holidays.setdefault(holiday.parent, []).append(holiday)

	return {
		shift: hl_holidays.get(holiday_list, [])
	...
	for employee, details in employee_details.items():
		...
		if filters.summarized_view:
			attendance = get_attendance_status_for_summarized_view(
				employee,
				filters,
				holidays + get_shift_holidays_for_employee(employee, attendance_map, shift_holiday_map),
			...
			attendance_for_employee = get_attendance_status_for_detailed_view(
				employee, filters, employee_attendance, holidays, shift_holiday_map
			)
...
def get_shift_holidays_for_employee(
	employee: str, attendance_map: dict, shift_holiday_map: dict
) -> list[dict]:
	employee_attendance = attendance_map.get(employee, {})
	holidays = []
	for shift in employee_attendance.keys():
		holidays.extend(shift_holiday_map.get(shift, []))
	return holidays
...
def get_attendance_status_for_detailed_view(
	...
	for shift, status_dict in employee_attendance.items():
		row = {"shift": shift}
		holidays_for_shift = shift_holiday_map.get(shift) or holidays
		"""{
	            'Morning Shift': {1: 'Present', 2: 'Absent', ...}
	            'Evening Shift': {1: 'Absent', 2: 'Present', ...}
	    },"""
		for d in total_days:
			...
			if status is None and holidays_for_shift:
				status = get_holiday_status(d, holidays_for_shift)
