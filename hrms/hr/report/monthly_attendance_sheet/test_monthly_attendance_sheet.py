...
class TestMonthlyAttendanceSheet(HRMSTestSuite):
	...
	def test_shift_type_holiday_list_in_detailed_view(self):
		previous_month_first = get_first_day_for_prev_month()
		year_start = getdate(get_year_start(previous_month_first))
		year_end = getdate(get_year_ending(previous_month_first))
		...
		holiday_list = make_holiday_list(
			"Test Shift Type Holiday List", from_date=year_start, to_date=year_end, add_weekly_offs=False
		)
		add_holiday_to_list(holiday_list, holiday_date)
		shift = setup_shift_type(shift_type="Test Shift Holiday Report", holiday_list=holiday_list)

		mark_attendance(self.employee, previous_month_first, "Present", shift.name)
