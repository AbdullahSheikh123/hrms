...
def get_holiday_dates_for_employee(
	employee: str,
	start_date: str,
	end_date: str,
	consider_shift_holiday_list: bool = False,
	fallback_holiday_list: str | None = None,
) -> list:
	"""
	Return employee holidays for a date range.

	When consider_shift_holiday_list is enabled, the holiday list attached to the
	active/default shift for a date takes precedence over employee/company holiday
	list assignment for that date.
	"""
	if not consider_shift_holiday_list:
		holiday_list = fallback_holiday_list or get_holiday_list_for_employee(employee)
	...
	start_date = getdate(start_date)
	end_date = getdate(end_date)
	fallback_holiday_list = fallback_holiday_list or get_holiday_list_for_employee(employee)
	fallback_holidays = set(get_holiday_dates_between(fallback_holiday_list, start_date, end_date))
	shift_holiday_cache = {}
	holiday_dates = set()
	...
	for day in range(no_of_days):
		...
		shift_name = shift_type.get("name") if shift_type else None
		shift_holiday_list = (
			frappe.get_cached_value("Shift Type", shift_name, "holiday_list") if shift_name else None
		)

		if shift_holiday_list:
			if shift_holiday_list not in shift_holiday_cache:
				shift_holiday_cache[shift_holiday_list] = set(
					get_holiday_dates_between(shift_holiday_list, start_date, end_date)
				)
			if holiday_date in shift_holiday_cache[shift_holiday_list]:
				holiday_dates.add(holiday_date)
		elif holiday_date in fallback_holidays:
			holiday_dates.add(holiday_date)
