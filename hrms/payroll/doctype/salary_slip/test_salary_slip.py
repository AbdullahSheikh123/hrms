...
class TestSalarySlip(HRMSTestSuite):
	...
	@HRMSTestSuite.change_settings("Payroll Settings", {"include_holidays_in_total_working_days": 0})
	def test_salary_slip_with_shift_type_holiday_list(self):
		from hrms.hr.doctype.shift_type.test_shift_type import setup_shift_type

		month_start_date = get_first_day(nowdate())
		month_end_date = get_last_day(nowdate())
		holiday_date = add_days(month_start_date, 1)
		while getdate(holiday_date).weekday() == 6:
			holiday_date = add_days(holiday_date, 1)

		fallback_holiday_list = make_holiday_list(
			"Test Payroll Fallback Holiday List",
			from_date=month_start_date,
			to_date=month_end_date,
			add_weekly_offs=False,
		)
		shift_holiday_list = make_holiday_list(
			"Test Payroll Shift Holiday List",
			from_date=month_start_date,
			to_date=month_end_date,
			add_weekly_offs=False,
		)
		holiday_list = frappe.get_doc("Holiday List", shift_holiday_list)
		holiday_list.append(
			"holidays",
			{"holiday_date": holiday_date, "description": "Shift Holiday", "weekly_off": 0},
		)
		holiday_list.save()

		emp_id = make_employee(
			"test_salary_slip_with_shift_holiday@salary.com",
			relieving_date=None,
			status="Active",
			company="_Test Company",
			holiday_list=fallback_holiday_list,
		)
		shift = setup_shift_type(
			shift_type="Test Payroll Shift Holiday", holiday_list=shift_holiday_list
		)
		frappe.db.set_value("Employee", emp_id, "default_shift", shift.name)

		ss = make_employee_salary_slip(
			emp_id,
			"Monthly",
			"Test Salary Slip With Shift Holidays Excluded",
		)

		days_in_month = date_diff(month_end_date, month_start_date) + 1
		self.assertEqual(ss.total_working_days, days_in_month - 1)
		self.assertEqual(ss.payment_days, days_in_month - 1)
